...
function OrgNode(){
	...
	this.inIt= function(){
		...
		tempHTML=tempHTML.replace("{NodeType}",this.NodeType);
...
function OrgShow(OrgNode_){
  ...
  this.click=null;
	this.Run=function(){
		...
		var sumNodes = [];
	  for(var i =0,len = this.DepthGroup[this.Depth].Nodes.length; i<len ; i++){//获取汇总node
      var sumNode = this.DepthGroup[this.Depth].Nodes[i].sumNode;
      if(sumNode!=null){
	      sumNode.sumChildNodes[sumNode.sumChildNodes.length] = this.DepthGroup[this.Depth].Nodes[i];
	      if(sumNodes.length==0){
	      	sumNodes[sumNodes.length]=sumNode;
	      }else{
	      	var exits = false;
	      	for(var j=0,sumLen=sumNodes.length;j<sumLen;j++){
	      		 if(sumNode.Id == sumNodes[j].Id){
	      		 	 exits = true;
	      		   break;	
	      		 }
	      	}
   ...
   for( var i =0,len=sumNodes.length;i<len;i++){//设置汇总节点距左，距顶距离
    	var sumNode = sumNodes[i]; 
    	BoxInit(sumNode);
	    //设置距顶距
	    var tempTop=sumNode.sumChildNodes[0].Top+this.IntervalHeight+sumNode.Height;
		  sumNode.Top=tempTop;
		  sumNode.Box.style.top=sumNode.Top + "px";
		  //设置距左距离
		  var tempLeft=(sumNode.sumChildNodes[0].Left+sumNode.sumChildNodes[sumNode.sumChildNodes.length-1].Left)/2;
		  sumNode.Left=tempLeft;
		  sumNode.Box.style.left=sumNode.Left + "px";
		}
		
		for(var i=0,len=sumNodes.length; i<len; i++){//设置汇总竖线条
			var tempNodes=sumNodes[i].sumChildNodes;
			for(var m=0; m<tempNodes.length; m++){//设置汇总child竖线条
				    var tempLineLeft=tempNodes[m].Left + (tempNodes[m].Width / 2);
						var tempLineHeight=((this.IntervalHeight - this.LineSize) / 2);
						var tempLineTop=tempNodes[m].Top + tempNodes[m].Height;
						var tempBottomLine=new CreateLine(2, tempLineTop, tempLineLeft, tempLineHeight, this.LineSize, this.LineColor, "SumLineBottom_"+ tempNodes[m].Id);
			}
			//设置汇总sum竖线条
			var tempLineLeft=sumNodes[i].Left + (sumNodes[i].Width / 2);
			var tempLineHeight=((this.IntervalHeight - this.LineSize) / 2);
			var tempLineTop=sumNodes[i].Top - 	tempLineHeight;
			var temptopLine=new CreateLine(2, tempLineTop, tempLineLeft, tempLineHeight, this.LineSize, this.LineColor, "SumLineTop_"+ sumNodes[i].Id);
			//设置横线条
			var tempLineWidth=(tempNodes[tempNodes.length-1].Left-tempNodes[0].Left)+ this.LineSize;
			var tempLineTop=tempNodes[0].Top + tempNodes[0].Height+((this.IntervalHeight - this.LineSize) / 2);
			var tempLineLeft=tempNodes[0].Left + (tempNodes[0].Width / 2);
			var tempGroupLine=new CreateLine(1, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "SumLineGroup_"+ sumNodes[i].Id);
	...
	function GetBoxTemplet(){//取得节点模板
		...
		var TempletStr="<div id=\"{Id}\" nodetype=\"{NodeType}\" class=\"OrgBox\" style=\""
		if(This.ShowType==2)TempletStr+="writing-mode: tb-rl;";
		TempletStr+="\" title=\"{Description}\" ";
		//TempletStr+="\" title=\"{Description}\" onclick=\""+This.click+"(this)\">{Text}</div>";
		if(This.click!=null)TempletStr+=" onclick=\""+This.click+"(this)\"";
		TempletStr+=">{Text}</div>";
		return TempletStr;
	}
