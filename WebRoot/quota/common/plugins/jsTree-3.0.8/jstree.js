 ...
 */
	...
	$.jstree.core.prototype = {
		...
		bind : function () {
			...
			this.element
				...
				.on("dblclick.jstree", ".jstree-anchor", $.proxy(function(e) {
					...
					this.dblclick_node(e.currentTarget, true, event);
				}, this))
		...
		dblclick_node: function(obj, check, e) {
			...
			this.trigger('dblclick_node',{'event':e,'node':obj});
		},
		...
		get_json : function (obj, options, flat) {
			...
			var tmp = {
				...
				'name' : obj.original.name ? obj.original.name : "", //80274970
				'menuId' : obj.original.menuId ? obj.original.menuId : "",
				'leaf'	:  obj.original.leaf,
				'url'	:  obj.original.url ? obj.original.url : "",
				'disabled'	:  obj.original.disabled,
				'old'	:  obj.original.old
				...
				*/
				...
				h2 = $("<"+"input />", {
						...
						"blur" : $.proxy(function () {
							...
							this.set_text(obj, t);
							/*80274970*/
							var flag = this.rename_node(obj, $('<div></div>').text(v)[this.settings.core.force_text ? 'text' : 'html']());
							if(flag){
								this.get_node(obj).original.name = this.get_node(obj).text;
							}
							if(flag === false) {
 ...
 */
	...
	$.jstree.plugins.search = function (options, parent) {
		...
		this.search = function (str, skip_async, show_only_matches) {
			...
			var MAX_SEARCH_RESULT_COUNT = 20; // IE8搜索运算慢, 且JS连续指令多会引发停止脚本警告. 搜索结果多了以后search_open和clear_search的DOM操作会很漫长, 因此限定搜索结果条数以规避IE8性能问题.
			var searchResultCount = 0;
			$.each(this._model.data, function (i, v) {
				if(v.text && ( (s.search_callback && s.search_callback.call(this, str, v)) || (!s.search_callback && (s.search_id ? f.search(v.id).isMatch : f.search(v.text).isMatch)) ) && (!s.search_leaves_only || (v.state.loaded && v.type == 'leaf')) ) {
					r.push(i);
					p = p.concat(v.parents);
					searchResultCount++;
				}
				if (searchResultCount >= MAX_SEARCH_RESULT_COUNT) {
				    return false;
				}
			});
			/**
			 ...
			 */
            if(searchResultCount >= MAX_SEARCH_RESULT_COUNT){
                JrzlTools.alert("符合条件的数据过多，仅展示其中"+searchResultCount+"个结果。请输入更精确的搜索条件。","提示",function(){
                    if(s.focusId != null){
                        $("#" + s.focusId).focus();
                    }
                });
            }
			if(r.length < 1 && s.search_null_tip){
				JrzlTools.alert("没有符合条件的数据","提示",function(){
					if(s.focusId != null){
						$("#" + s.focusId).focus();
					}
				});
			...
			if(r.length) {
				p = $.vakata.array_unique(p);
				this._search_open(p);
				this._data.search.dom = $(this.element.find('#' + $.map(r, function (v) { return "0123456789".indexOf(v[0]) !== -1 ? '\\3' + v[0] + ' ' + v.substr(1).replace($.jstree.idregex,'\\$&') : v.replace($.jstree.idregex,'\\$&'); }).join(', #')));
	...
	(function ($) {
		...
		$.vakata.search = function(pattern, txt, options) {
			...
			search = function (text) {
				...
				if(pattern === text || text.indexOf(pattern) !== -1) { // 搜索到了
					return {
						isMatch: true,
						score: 0
					};
				}
				if(!options.fuzzy && typeof pinyinMakePy === "function") { //未找到 且不使用fuzzy
					var flag = false;
					var matcher = new RegExp(pattern, "i" );
					var pinyinList = pinyinMakePy(text);
